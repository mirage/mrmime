...
let example0 =
  ...
  let header0 =
    ...
    Header.of_list Field.[ Field (Field_name.content_type, Content, content0)
                         ; Field (Field_name.content_encoding, Encoding, `Base64) ] in
  ...
  let subject =
    ...
    compile [ v "First"; sp 1; v "email" ] in
  ...
  let header =
    ...
    :: Field.(Field (Field_name.v "To", Addresses, Address.[ mailbox thomas; mailbox anil ]))
...
let example1 =
  ...
  let header0 =
    let content0 =
      let open Content_type in
      make `Text (Subtype.v `Text "plain") Parameters.(of_list [ k "charset", v "utf-8" ]) in
    Header.of_list Field.[ Field (Field_name.content_type, Content, content0) ] in

  let subject =
    let open Unstructured.Craft in
    compile [ v "Second"; sp 1; v "email" ] in
  ...
  let header =
    Field.(Field (Field_name.sender, Mailbox, john))
    :: Field.(Field (Field_name.v "To", Addresses, Address.[ mailbox thomas; mailbox anil; mailbox hannes; mailbox gemma ]))
    :: Field.(Field (Field_name.subject, Unstructured, subject))
    :: Field.(Field (Field_name.date, Date, now))
    :: [] in
